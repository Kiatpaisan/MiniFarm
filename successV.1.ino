...
//netpie
#define APPID   "MiniFarm"
#define KEY     "jfJBUo7iTdU0TX5" // device1
#define SECRET  "UXfIGCt7PV3YSLo3GbmNSBQCi"
#define ALIAS   "esp8266"
#define FEEDID   "FEEDV1"           // ***** FeedID
#define FEEDAPI  "vohZ4GOTfbnahyNDVEx9XN9nNNqYxBzB"        //  FeedAPI
#define TARGETNAME "MiniFarmfreeboard"
...
int settemp = 27 ;  //อุณภูมิที่กำหนด
...
void ledOn(int pin1, int pin2, int pin3) { // R Y G (แสดงไฟสถานะ)
  digitalWrite(D0, 0);
  digitalWrite(D5, 0);
  digitalWrite(D6, 0);

  digitalWrite(D0, pin1);
  digitalWrite(D5, pin2);
  digitalWrite(D6, pin3);
}
...
void OLEDPRINT () {  //แสดงสถานะทำงาน
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    OLED.print("MiniFarm Config to 192.168.4.1");
  }
  if (microgear.connected()) {
